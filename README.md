...
- **리뷰 반영** — 멘토 리뷰 뒤 문서와 코드의 숫자 두 곳을 맞추고(상대 import 28건, 캐시 복원 시간), origin을 상수에서 함수로 바꿔 브라우저 번들이 서버 전용 env를 찾지 않게 했습니다. 그 결과 `NEXT_PUBLIC_BASE_URL`과 env 규칙 ②가 필요 없어졌습니다(게이트로 막던 사고를 구조로 제거). 승격한 ESLint 룰과 워크플로 불변 조건에 회귀 테스트를 붙이고, production 배포 뒤 스모크 E2E를 자동화했습니다.
