...
- **리뷰 반영** — 멘토 리뷰 뒤 문서와 코드의 숫자를 맞추고(상대 import 28건, 캐시 복원 시간 등 넷), origin을 역할로 갈랐습니다. 사이트 origin(`APP_ORIGIN`)은 og:url·canonical에만 쓰고, 서버 self-fetch는 들어온 요청의 origin과 쿠키를 씁니다. 값 하나가 두 역할을 겸하던 걸 나누자 게이트 둘이 필요 없어졌습니다 — 브라우저가 origin을 찾지 않게 되어 `NEXT_PUBLIC_BASE_URL`과 env 규칙 ②가 사라졌고, preview가 production API를 부를 수 없게 되어 그걸 막으려던 규칙도 필요 없어졌습니다. 승격한 ESLint 룰·워크플로 불변 조건·서버 self-fetch에 회귀 테스트를 붙이고, production 배포 뒤 스모크 E2E를 자동화했습니다. 새로 만든 테스트에는 손으로 뮤테이션을 넣어 빨간불이 나는지 확인했고, 그 과정에서 실패할 수 없던 테스트 둘을 고쳤습니다.
