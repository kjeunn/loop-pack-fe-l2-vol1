...
- **예산 게이트** — Next 16이 First Load JS를 안 찍어 매니페스트로 라우트별 청크를 조립해 size-limit에 넣었고, 한도는 7주차 SHA 재측정값과 현재값 위에 +10 kB(정상 성장의 두 배, 라이브러리 하나보다 작음)로 계산합니다. 환경 변수는 이 앱에서 날 수 있는 사고 다섯 개에서 규칙을 역산해 빌드 앞 step과 `next.config.ts` 양쪽에서 검증합니다. 실험 PR 세 개(번들 초과·잘못된 env·상대 import)로 빨간불과 PR 화면의 원인 표를 확인했습니다.
- **AI 리뷰** — 팀 컨벤션을 규칙 ID로 옮긴 리뷰 스킬을 만들고 같은 diff에 앵커 유무로 두 번 돌렸습니다. 앵커가 노이즈(15 → 6건)와 함께 실패 경로 결함 5건도 걸러낸 걸 확인해, 프롬프트에 "결함 후보" 섹션과 "미사용 주장은 Grep 확인 뒤에만" 규칙을 더했습니다.
