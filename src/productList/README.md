...
3. **`?page=99`처럼 마지막 페이지를 넘는 URL이 빈 화면**
   재현: 주소창에 `?page=99` 입력. 원인: 존재하지 않는 페이지라 결과가 0개인데 클램프가 없었다. 수정: 첫 응답으로 실제 페이지 수를 안 뒤 **첫 페이지로 되돌린다**(필터 리셋과 같은 정상 진입점 — 첫 로딩 이후에만 판정).
