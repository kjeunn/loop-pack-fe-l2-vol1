...
> **용도: 분석·학습.** 우리 공통 컴포넌트 설계 리뷰와 외부 라이브러리 소스 학습(왜 이렇게 짰나·무엇을 빌릴까)에 쓴다.
> 레포 컨벤션(패턴 계약·FSD 배치) 준수 **검사**는 `pattern-review`가 담당한다.
...
- **책임 범위** — 컴포넌트/훅/store/service를 각각 한 문장으로. "그리고"가 많으면 분리 후보. (관심사 분리·Custom Hook 책임은 이 관점이 담당한다.)
