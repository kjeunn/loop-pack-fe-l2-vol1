...
**배포 뒤 확인은 아직 손이다.** 위 스모크는 손으로 돌린 것이라 다음 배포에서 빠질 수 있다. `deployment_status`(production, success)를 받아 배포된 주소에 E2E를 돌리는 워크플로를 썼다가 커밋하지 않았다. 두 가지 때문이다. 첫째, 이 E2E는 `order.spec.ts`가 실제로 주문을 만든다(응답의 주문 id로 단언한다). 배포마다 production에 테스트 주문을 쌓는 건 [7-3](#7-생각해-볼-질문)에서 경계한 바로 그 일이고, 지금 안 터지는 건 백엔드가 mock이라서지 설계가 안전해서가 아니다. 둘째, `environment_url`이 production 도메인인지와 `deployment.environment`의 값이 무엇인지를 배포 없이는 확인할 수 없다. 붙일 때의 조건은 셋이다 — 쓰기 없는 스펙(`home`·`products`·`dialog`·`select`)만 돌릴 것, 위 두 값을 첫 실행에서 확인할 것, `environment_url`이 배포별 생성 URL로 오면 Standard Protection에 막히므로 주소를 고정할 것. 워크플로 파일 자체는 `quality-workflow.test.mts`가 불변 조건(SHA 핀·읽기 권한·`pull_request_target` 부재·checkout 자격 증명 미보존·timeout·artifact 범위·e2e 실행 조건)을 고정한다. 실행돼야 드러나는 동작은 실험 PR로, 빠지면 조용히 약해지는 조건은 테스트로 본다.
