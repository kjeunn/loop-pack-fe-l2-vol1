...
`server-only`는 붙이지 못했다. `serverRequest.ts`는 `fetcher.ts`가 동적으로 부르는데 `fetcher.ts`는 클라이언트 컴포넌트도 쓰므로 클라이언트 SSR 그래프에 들어가 빌드가 막혔다. 대신 호출자가 `typeof window`로 막고, 브라우저에 실리지 않는 것은 빌드 산출물로 확인했다 — `.next/static`에 `next/headers`·`x-forwarded-proto`·`readServerRequestContext` 0건, 같은 방식으로 찾은 대조 문자열은 2건.
